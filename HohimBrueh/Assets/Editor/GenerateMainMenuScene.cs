...
namespace FrogSmashers.Editor
{
    public static class MainMenuGenerator
    {
        ...
        public static void Generate()
        {
            ...
            var openBtn     = BuildMenuButton(canvas.transform, "OpenLobbiesButton", "OPEN LOBBIES",  2,  font);
            var rankedBtn   = BuildMenuButton(canvas.transform, "RankedQueueButton", "RANKED QUEUE",  3,  font);
            var settingsBtn = BuildMenuButton(canvas.transform, "SettingsButton",    "SETTINGS",      4,  font);

            var (panel, panelText, backBtn) = BuildComingSoonPanel(canvas.transform, font);

            SetupNavigation(localBtn, createBtn, openBtn, rankedBtn, settingsBtn);

            var controllerGo = new GameObject("MainMenuController");
            var controller = controllerGo.AddComponent<MainMenuController>();
            controller.localGameButton    = localBtn;
            controller.createLobbyButton  = createBtn;
            controller.openLobbiesButton  = openBtn;
            ...
            WirePersistent(openBtn.onClick,     controller, nameof(MainMenuController.OnOpenLobbies));
